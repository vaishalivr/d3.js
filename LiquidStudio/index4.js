const render4 = data => {
	...
	g.selectAll('.bar')
		...
		.attr('y', d => yScale(d.smartphoneonly))
		.attr('width', xScale.bandwidth())
		.attr('height', d => innerHeight - yScale(d.smartphoneonly))
		.attr('class', 'bar')
		.attr('id', d => ('rect-4-'+ d.year))
		.attr('fill', '#ccc')
	...
	g.append('g').
		...
		.attr('y', d => yScale(d.smartphoneonly) - 9)
		.attr('fill', 'black')
	...
	const slider = d3.sliderBottom()
		...
		.on('onchange', d => {
			...
			d3.select('#indicator4').select('line')
				.attr('x1', xScale(yearonslider) + xScale.bandwidth()/2)
				.attr('x2', xScale(yearonslider) + xScale.bandwidth()/2)

			d3.select('#indicator4').select('text')
				.attr('x', xScale(yearonslider) - dataasdictionary[yearonslider].smartphoneonly.length/2 + 4)
				.text(dataasdictionary[yearonslider].smartphoneonly)
				.attr('dy',9)
	...
	g.append('text')
		.text('Number of Smart Phone only Internet users')
		.attr('text-anchor', 'middle')
		.attr('x', innerWidth/2)
		.attr('y', 310)
		.attr('font-size', 15)
