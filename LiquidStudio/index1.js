...
const render1 = data => {
	...
	const subgroups = ['smarttv', 'normaltv']
	...
	/*const totals = [96.5, 95.9, 96.1, 96.2, 96.95, 97.88, 98.82]*/

	/*const text0 = g.append('g').selectAll('text')
		.data(data)
		.enter()
		.append('text')
		.attr('id', d => 'text' + d.year)
		.text(d => d.total)
	...
	const text1 = g.append('g').selectAll('text')
		...
		.enter()
		.append('text')
		.attr('id', d => 'text1' + d.year)
		.text(d => d.smarttv)
		.attr('text-anchor', 'middle')
		.attr('x', d => xScale(d.year) + xScale.bandwidth()/2)
		.attr('y', d => yScale(d.smarttv) + 12)
		.style('font-size', '12px')
		.style('fill', '#ccc')
		...
		.on('onchange', d => {
			...
			for (var index=0; index < stackedData[0].length; index++) {
				...
				if (yearindata <= yearonslider) {
					...
					/*d3.select('#text' + yearindata)
						.style('fill', 'black')*/

					d3.select('#text1' + yearindata)
						.style('fill', 'black')

					d3.select('#text2' + yearindata)
						.style('fill', 'black')
	...
	const addSlider = d3.select('#slider-step1')
		.append('svg')
		.attr('width', 300)
		.attr('height', 100)
		.append('g')
	...
	g.append('text')
		.text('Number of Households with Television')
		.attr('text-anchor', 'middle')
		.attr('x', innerWidth/2)
		.attr('y', 310)
		.attr('font-size', 15)
